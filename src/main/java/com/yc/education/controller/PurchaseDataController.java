...
public class PurchaseDataController extends BaseController implements Initializable {
    ...
    public  void  changeClick(String value){
        ...
        if(value.equals("询价单")){url= "/fxml/purchase/inquiry.fxml";}

        if(value.equals("采购订单")){url="/fxml/purchase/purchase_order.fxml";}

        if(value.equals("强制结案")){url="/fxml/purchase/constraint.fxml";}

        if(value.equals("在途库存")){url="/fxml/purchase/ontheway.fxml";}

        if(value.equals("应付账款")){url="/fxml/purchase/pay.fxml";}

        if(value.equals("采购未入库")){url="/fxml/purchase/nostorage.fxml";}

        if(value.equals("产品库存查询")){url="/fxml/purchase/product_stock.fxml";}

        if(value.equals("最新采购查询")){url="/fxml/purchase/purchase_query.fxml";}
