...
//        loadData();
    ...
    public void find(){
        ...
        List<SaleGoodsProduct> list = iSaleGoodsProductService.listSaleGoodsProductDiscount(customerNo, customerNoEnd, productNo, productNoEnd, productName, productNameEnd, saleDateStr, saleDateEndStr, customerName, customerNameEnd, discountStr);
        if(list != null){
            //todo.. 渲染数据
            loadData(list);
        }
    ...
    private void loadData(List<SaleGoodsProduct> list){
        ...
        col_customer_discount.setCellValueFactory(new PropertyValueFactory(""));
